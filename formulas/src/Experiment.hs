...
timeRatioLhsKV :: Double -> Double -> [StatComp] -> Map (Double, Double) [Double]
timeRatioLhsKV epsK epsV stats = 
  (getStatCompTimeRatio <<$>> Fold.fold f stats)
  `Map.union` 
  Map.fromList [((bin epsK k, bin epsV v), []) | k <- [1.0, 1.0 + 2 * epsK..1000.0], v <- [0.5, 0.5 + 2 * epsV .. 99.5]] 
  where
    f = Fold.groupBy key Fold.list
    key stat = (bin epsK . fromIntegral . getCompParamsParallel . getStatCompParams $ stat, 
                bin epsV . getCompParamsVarRunTime . getStatCompParams $ stat)
    bin e = Statistics.toBinEpsilon 0 e


---- Stats Dist Time Ratio LHS K V ----

data DistTimeRatioLhsKV = DistTimeRatioLhsKV
   Double -- parallel
   Double -- nGen
   Double -- minimum
   Double -- mean

distTimeRatioLhsKV :: Map (Double, Double) [Double] -> [DataSet DistTimeRatioLhsKV]
distTimeRatioLhsKV stats = Fold.fold f (Map.toList stats)
                         & (fmap snd . Map.toAscList)
                       <&> (DataSet . mconcat . fmap snd . Map.toAscList)
  where
    f :: Fold ((Double, Double), [Double]) (Map Double (Map Double [DistTimeRatioLhsKV]))
    f = 
          Fold.groupBy (\((k, _), _) -> k) 
        . Fold.groupBy (\((_, v), _) -> v)
        $ Fold.list
        & (fmap . fmap) 
            (\((k, v), trs) -> Fold.fold (foldSubset k v) trs)
    foldSubset :: Double -> Double -> Fold Double DistTimeRatioLhsKV
    foldSubset k v = DistTimeRatioLhsKV k v 
        <$> (Fold.minimum 
              <&> fromMaybe (panic $ "distTimeRatioLhsKV: no value to compute minimum (k = " <> show k <> ", v = " <> show v <> " )."))
        <*> Fold.mean

---- Stats Time Ratio LHS K NGen ----

timeRatioLhsKNGen :: Double -> Double -> [StatComp] -> Map (Double, Double) [Double]
timeRatioLhsKNGen epsK epsNGen stats = 
  (getStatCompTimeRatio <<$>> Fold.fold f stats)
  `Map.union` 
  Map.fromList [((bin epsK k, bin epsNGen g), []) | k <- [1.0, 1.0 + 2 * epsK ..1000.0], g <- [1, 1 + 2 * epsNGen .. 10^5]] 
  where
    f = Fold.groupBy key Fold.list
    key stat = (bin epsK . fromIntegral . getCompParamsParallel . getStatCompParams $ stat, 
                bin epsNGen . fromIntegral . getCompParamsNGen . getStatCompParams $ stat)
    bin e = Statistics.toBinEpsilon 0 e

---- Stats Dist Time Ratio LHS K NGen ----

data DistTimeRatioLhsKNGen = DistTimeRatioLhsKNGen
   Double -- parallel
   Double -- nGen
   Double -- minimum
   Double -- mean

distTimeRatioLhsKNGen :: Map (Double, Double) [Double] -> [DataSet DistTimeRatioLhsKNGen]
distTimeRatioLhsKNGen stats = Fold.fold f (Map.toList stats)
                         & (fmap snd . Map.toAscList)
                       <&> (DataSet . mconcat . fmap snd . Map.toAscList)
  where
    f :: Fold ((Double, Double), [Double]) (Map Double (Map Double [DistTimeRatioLhsKNGen]))
    f = 
          Fold.groupBy (\((k, _), _) -> k) 
        . Fold.groupBy (\((_, g), _) -> g)
        $ Fold.list
        & (fmap . fmap) 
            (\((k, g), trs) -> Fold.fold (foldSubset k g) trs)
    foldSubset :: Double -> Double -> Fold Double DistTimeRatioLhsKNGen
    foldSubset k g = DistTimeRatioLhsKNGen k g 
        <$> (Fold.minimum 
              <&> fromMaybe (panic "distTimeRatioLhsKNGen: no value to compute minimum."))
        <*> Fold.mean
